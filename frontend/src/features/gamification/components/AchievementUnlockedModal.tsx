...
interface AchievementUnlockedModalProps {
  ...
  onClose: () => void;
  onShare?: (type: 'twitter' | 'facebook' | 'whatsapp') => void;
  isOpen: boolean;
}
...
const AchievementUnlockedModal: React.FC<AchievementUnlockedModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50">
      ...
      {/* Conteúdo do modal */}
      <div 
        className="relative bg-white rounded-lg shadow-lg overflow-hidden max-w-md w-full mx-4 animate-fadeIn"
        onClick={e => e.stopPropagation()}
      >
        {/* Header com cor de destaque */}
        <div className="bg-gradient-to-r from-amber-500 to-yellow-400 p-5 text-center">
          <h2 className="text-white text-xl font-bold mb-1">
            Nova Conquista Desbloqueada!
          </h2>
          <p className="text-amber-100 text-sm">
            Parabéns por sua jornada espiritual!
          </p>
        </div>
        ...
        <div className="p-6">
          ...
          <div className="flex flex-col gap-3">
            ...
            <button
              onClick={onClose}
              className="w-full mt-4 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-md font-medium transition-colors"
            >
              Continuar Jornada
            </button>
          </div>
